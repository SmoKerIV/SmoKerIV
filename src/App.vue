  ...
  >
    ...
    >
      ...
      <div class="flex items-center gap-1.5">
        ...
        <button
          @click="closeWindow"
          class="w-3 h-3 rounded-full bg-[#ff5f57] opacity-70 hover:opacity-100 transition-opacity cursor-pointer"
        ></button>
      </div>
    </div>
    ...
    <Transition name="fade">
      ...
      >
        ...
        >
          ...
          >
            <div class="flex items-center gap-2 text-[#cccccc]">
              ...
              <span class="text-[11px] uppercase tracking-wider"
                >Terminal — bash (sudo)</span
              >
            </div>
            <button
              @click="closeSudoModal"
          ...
          <div
            ref="terminalBodyRef"
            class="flex-1 overflow-y-auto p-3 editor-scroll"
          >
            ...
            <template v-for="(entry, i) in sudoHistory" :key="i">
              ...
              <div
                :class="
                  entry.type === 'error' ? 'text-[#f44747]' : 'text-[#cccccc]'
                "
            ...
            <div class="flex items-center gap-1 mt-1">
              ...
              <input
                ref="sudoInputEl"
                v-model="sudoInput"
                @keydown.enter.prevent="runCheatCode"
                @keydown.escape.prevent="closeSudoModal"
                class="bg-transparent outline-none text-[#cccccc] flex-1 caret-[#cccccc] ml-1"
                autocomplete="off"
                spellcheck="false"
    ...
    <Transition name="fade">
      ...
      >
        <div class="flex justify-between items-start mb-2">
          <span
            class="text-[#4ec9b0] text-[11px] uppercase tracking-wider font-bold"
            >● Notification</span
          >
          <button
            @click="hireToast = false"
            class="text-[#969696] hover:text-white leading-none ml-4"
          >
            ×
          </button>
        </div>
    ...
    >
      <div class="flex items-center gap-3">
        <div class="relative">
          ...
          >
            <div
              class="px-3 py-1 text-[#569cd6] border-b border-[#3c3c3c] mb-1 flex justify-between items-center"
            >
              <span class="font-mono">git log --oneline main</span>
              <button
                @click.stop="showGitLog = false"
                class="text-[#969696] hover:text-white ml-4"
              >
                ×
              </button>
            </div>
